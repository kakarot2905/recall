...
export async function PUT(req: NextRequest) {
  ...
  const existingTodayStats = Array.isArray(existing?.todayStats)
    ? existing.todayStats
    : [];
  const existingGhostCardShown = toPlainObject(existing?.ghostCardShown);
  const existingSeenFirstCardPerSource = toPlainObject(
    existing?.seenFirstCardPerSource,
  );
  ...
  if (
    payload.sm2State &&
    typeof payload.sm2State === "object" &&
    !Array.isArray(payload.sm2State)
  ) {
    Object.entries(payload.sm2State).forEach(
      ([cardId, incomingEntry]: [string, any]) => {
        ...
        const existingLastReviewed = toDateTimestamp(
          existingEntry.lastReviewed,
        );
        const incomingLastReviewed = toDateTimestamp(
          incomingEntry?.lastReviewed,
        );
  ...
  if (
    payload.ghostCardShown &&
    typeof payload.ghostCardShown === "object" &&
    !Array.isArray(payload.ghostCardShown)
  ) {
    Object.entries(payload.ghostCardShown).forEach(
      ([key, incomingValue]: [string, any]) => {
        mergedGhostCardShown[key] =
          mergedGhostCardShown[key] === true ? true : Boolean(incomingValue);
      },
    );
  }
  ...
  if (
    payload.seenFirstCardPerSource &&
    typeof payload.seenFirstCardPerSource === "object" &&
    !Array.isArray(payload.seenFirstCardPerSource)
  ) {
    Object.entries(payload.seenFirstCardPerSource).forEach(
      ([key, incomingValue]: [string, any]) => {
        mergedSeenFirstCardPerSource[key] =
          mergedSeenFirstCardPerSource[key] === true
            ? true
