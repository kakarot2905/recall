...
export async function PUT(req: NextRequest, { params }: { params: { sourceId: string } }) {
    ...
    if (typeof body.examDate !== 'undefined') source.examDate = parseOptionalExamDate(body.examDate)
    if (status2 && ['pending', 'processing', 'done', 'failed'].includes(status2)) source.status = status2
