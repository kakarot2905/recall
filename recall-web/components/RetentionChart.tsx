...
function buildSourceTopicMap(sources: any[]) {
    ...
    for (const source of sources) {
        if (source?._id) map[String(source._id)] = source.topic || String(source._id)
    }
    return map
}
...
function computeRetentionSeries(cards: any[], sm2State: Record<string, any>, sources: any[]) {
...
export default function RetentionChart({ cards, sources, sm2State }: Props) {
    ...
    const { labels, series, dayOffsets } = computeRetentionSeries(cards, sm2State, sources)
    ...
    return (
        <div>
            <div style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 12, color: '#5e6678', display: 'block', marginBottom: 4 }}>
                    Topic
                </label>
                <select
                    value={selectedTopic}
                    onChange={e => setSelectedTopic(e.target.value)}
                    style={{ border: '1px solid #d8deea', borderRadius: 8, padding: '6px 8px', fontSize: 13, fontFamily: 'inherit', background: '#fff', minWidth: 180 }}
                >
