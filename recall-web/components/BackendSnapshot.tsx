...
function countCardTypes(cards: any[]) {
  const counts = { mcq: 0, short_answer: 0, fill_blank: 0, fact: 0 } as Record<
    string,
    number
  >;
    ...
    <div>
      <div style={sectionStyle}>
        ...
        <div style={{ padding: "4px 0" }}>
          <div style={rowStyle}>
            <span style={keyStyle}>User</span>
            <span>
              {user?.name} ({user?.email})
            </span>
          </div>
          <div style={rowStyle}>
            <span style={keyStyle}>Total Sources</span>
            <span>{sources.length}</span>
          </div>
          <div style={rowStyle}>
            <span style={keyStyle}>Total Cards</span>
            <span>
              {cards.length} — mcq: {totalCounts.mcq}, short_answer:{" "}
              {totalCounts.short_answer}, fill_blank: {totalCounts.fill_blank},
              fact: {totalCounts.fact}
            </span>
          </div>
        </div>
      </div>
        ...
        return (
          <div key={source._id} style={sectionStyle}>
            <div style={titleStyle}>
              {source.topic} —{" "}
              <span style={{ fontWeight: 400 }}>{source.status}</span> —{" "}
              {sourceCards.length} card(s)
            </div>
            <div style={{ padding: "4px 0" }}>
              <div style={rowStyle}>
                <span style={keyStyle}>Exam Date</span>
                <span>
                  {source.examDate
              ...
              <div style={rowStyle}>
                <span style={keyStyle}>Notes Preview</span>
                <span style={{ fontSize: 12, color: "#5e6678" }}>
                  {source.notes?.slice(0, 80)}
                </span>
              </div>
              <div style={rowStyle}>
                <span style={keyStyle}>Card Types</span>
                <span style={{ fontSize: 12, color: "#5e6678" }}>
                  mcq: {counts.mcq}, short_answer: {counts.short_answer},
                  fill_blank: {counts.fill_blank}, fact: {counts.fact}
                </span>
              </div>
            </div>
          </div>
