export const CARD_TYPES = new Set(['mcq', 'short_answer', 'fill_blank', 'fact'])
...
export function normalizeCardPayload(body: any) {
    ...
    const options = Array.isArray(body.options)
        ...
        : toTrimmedString(body.options)
            ? toTrimmedString(body.options).split(',').map((o: string) => o.trim()).filter(Boolean)
            : []

    return {
        type, question, content, correct, answer, difficulty, options,
...
export function validateNormalizedCardPayload(normalized: any): string | null {
    ...
    if (normalized.type === 'fact' && !normalized.content) return 'Fact cards require content'
    if (normalized.type !== 'fact' && !normalized.question) return 'This card type requires a question'
    return null
}